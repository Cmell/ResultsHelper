...
getOpts <- function (arguments=NULL) {
  ...
  #' 
  #' @param arguments A character vector of arguments to access. Otherwise,
  ...
  for (arg in arguments) {
    if (exists(arg, where = parent.frame())) {
      if (is.null(get(arg, envir = parent.frame()))) {
        # if it is null, we need to assign it.
        assign(arg, get(arg, envir = optEnv), envir=parent.frame())
      } else {
        # if it exists and is not null, nothing needs to be done. Just break
        # out.
        break
      }
    } else {
      # If it does not exist, then assign it.
      assign(arg, get(arg, envir = optEnv), envir=parent.frame())
    }
  }
}
...
fStr <- function (param, modObj, digits=NULL, asEqn=NULL, dfDigits=NULL) {
...
pStr <- function (param, modObj, asEqn=NULL, pDigits=NULL) {
...
tStr <- function (param, modObj, digits=NULL, dfDigits=NULL) {
